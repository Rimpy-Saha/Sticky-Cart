  ...
  Drupal.eu_cookie_compliance.createWithdrawBanner = function (html) {
    ...
    if (drupalSettings.eu_cookie_compliance.popup_position) {
      ...
      setTimeout(function () {
        ...
        $html.css({ top: Drupal.eu_cookie_compliance.getBannerTopHiddenPosition(height) });
  ...
  Drupal.eu_cookie_compliance.toggleWithdrawBanner = function () {
    ...
    if (drupalSettings.eu_cookie_compliance.popup_position) {
      if (bannerIsShowing) {
        ...
        $wrapper.css({ top: Drupal.eu_cookie_compliance.getBannerTopHiddenPosition(height) }).trigger('eu_cookie_compliance_popup_close');
      }
      else {
        ...
        $wrapper.css({ top: Drupal.eu_cookie_compliance.getBannerTopPosition(height) }).trigger('eu_cookie_compliance_popup_open');
      }
    }
    else {
      if (bannerIsShowing) {
        // $wrapper.animate({ 'bottom': -1 * (height) }, drupalSettings.eu_cookie_compliance.popup_delay).trigger('eu_cookie_compliance_popup_close');
        $wrapper.css({ 'bottom': -1 * (height) }).trigger('eu_cookie_compliance_popup_close');
      }
      else {
        // If "Do not show cookie policy when the user clicks the Cookie policy button." is
        // selected, the inner banner may be hidden.
        ...
        $wrapper.css({ 'bottom': 0 }).trigger('eu_cookie_compliance_popup_open');
      }
    }
    if (bannerIsShowing) {
      $('body').removeClass('eu-cookie-compliance-popup-open');
